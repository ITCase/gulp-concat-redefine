...
describe('get_files', function(){
    ...
    function check_files (files, key, right_files) {
        expect(files).to.have.property(key).that.is.an('array')
...
describe('get_target', function(){
    it('should set target file', function(){
        for(var app_name in cr.files) {
            expect(cr.get_target(app_name)).to.be.a('string').to.contain(cr.opts.target_prefix)
                                           .that.contain(app_name).that.contain(cr.opts.type);
        }
    });
});
